...
def enable_subwoofer():
  call(["sudo", "hda-verb", dev, "0x17", "SET_POWER", "0x0"])
  call(["sudo", "hda-verb", dev, "0x1a", "SET_POWER", "0x0"])

  call(["sudo", "hda-verb", dev, "0x17", "0x300", "0xb000"])
  call(["sudo", "hda-verb", dev, "0x17", "0x707", "0x40"])
  call(["sudo", "hda-verb", dev, "0x1a", "0x707", "0x25"])

def disable_subwoofer():
  call(["sudo", "hda-verb", dev, "0x1a", "0x707", "0x20"])

def set_subwoofer_volume(volumes):
  valL = 0xa000 + volumes[0]
  valR = 0x9000 + volumes[1]
  call(["sudo", "hda-verb", dev, "0x03", "0x300", hex(valL)])
  call(["sudo", "hda-verb", dev, "0x03", "0x300", hex(valR)])
...
def set_speaker_volumes(volumes):
  ...
  call(["amixer", "-D", "pulse", "set", "Master", str(volumes[0]) + "%," + str(volumes[1]) + "%"])
